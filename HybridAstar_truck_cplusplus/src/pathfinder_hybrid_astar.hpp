...
Path_Final* calc_hybrid_astar_path(double sx, double sy, double syaw, double syaw1, double gx, double gy, double gyaw, double gyaw1, Map& map) {
   ...
   /*------ debug -------*/
   // check map dimension/idx
   /*--- debug ---*/
   cout<<"("<<map.minx<<","<< map.maxx << ") with length "<< map.xw <<"* "<<"("<<map.miny<<","<< map.maxy << ") with length "<< map.yw<<endl;
   cout<<"("<<nstart.xind<<","<<nstart.yind << ") ------- start node"<<endl;
   cout<<"("<<ngoal.xind<<","<<ngoal.yind << ") ------- goal node"<<endl;
   // ------------------------
    ...
    /*---- debug -----*/
    cout<<"start node idx: -->"<<map.calc_index(nstart)<<endl;
    

    // In pq, the first element is cost and the second element is the index of grid position
    priority_queue<pair<double, int>,
               std::vector<pair<double, int> >, // just the standard form for customized pq
	...
	//cout<<"nmotion: -->"<<nmotion<<endl;
        /*--- debug ---*/
	//cout<<"open set size: -->"<<open_set.size()<<endl;
		...
		ct_num++;
            ...
            /*--- debug ---*/
	    //cout<<"cur node idx: -->"<<node_ind<<endl;
        ...
        /*--debug --*/
        //cout<<"# of invalid nodes: "<<ct_num<<endl;
    }
...
vector<Node> update_node_with_analystic_expantion(Node& current, Node& ngoal, Map& map, double gyaw1) {
    ...
    if (!finalpath.empty()) {
        ...
        /*---  IMPORTANT: Check Trailer yaw angle error!!! ---*/ 
