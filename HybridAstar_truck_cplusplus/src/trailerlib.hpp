...
bool check_collision(vector<double>& x, vector<double>& y, vector<double>& yaw, Map& map, const double wbd, const double wbr, const vector<double>& vrx, const vector<double>& vry);
bool rect_check(double ix, double iy, double iyaw, vector<double>& ox, vector<double>& oy, const vector<double>& vrx, const vector<double>& vry);
...
bool check_collision(vector<double>& x, vector<double>& y, vector<double>& yaw, Map& map, const double wbd, const double wbr, const vector<double>& vrx, const vector<double>& vry) {
    ...
    x,y,yaw: path info
    map: obstacle map
    wbd, wbr: bubble size parameters (collision check)
    */
    ...
    for (size_t i = 0; i < x.size(); i++) {
        ...
        /*  case 1: bubble check only */ 
        /*
	const std::vector<int> radIndices = map.kdtree.radiusSearch({cx, cy}, wbr);
        if (!radIndices.empty()) {
	...
	*/
        ...
        /* case 2: bubble check + rect check*/
	const std::vector<int> radIndices = map.kdtree.radiusSearch({cx, cy}, wbr);
	// bubble check passed
	if (radIndices.empty()) 
		continue;
	...
	// rectangular check fail
        if (!rect_check(ix, iy, iyaw, oxnew, oynew, vrx, vry))
             return false; // collision
...
bool check_trailer_collision(Map& map, vector<double>& x, vector<double>& y, vector<double>& yaw0, vector<double>& yaw1) {
    ...
    */
    ...
    if (!check_collision(x, y, yaw1, map, DT, DTR, VRXT, VRYT)) {
         return false;
    }

    // check collision for front truck
    if (!check_collision(x, y, yaw0, map, DF, DFR, VRXF, VRYF)){
        return false;
    }
